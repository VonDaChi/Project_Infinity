...
async def select_model(input_session):
    console.print(Panel(f"[bold magenta]{tr('entry.llm_title', suffix=' (Claude)')}[/bold magenta]", expand=False))
...
async def main():
    ...
    if not api_key:
        console.print(f"[bold red]{tr('err.prefix')}[/bold red] {tr('entry.apikey_missing', key='ANTHROPIC_API_KEY')}")
        console.print(f"[yellow]{tr('entry.apikey_hint', key='ANTHROPIC_API_KEY')}[/yellow]")
        sys.exit(1)
